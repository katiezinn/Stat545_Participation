...
```
#quiets the library callout on your code
...
gapminder %>%
  ...
  filter(country == "Canada") #just so we can look at simply canada
```
...
gapminder %>%
  ...
  summarize(mu = mean(lifeExp),
            md = median(lifeExp)) #gives same thing but leaves the continent column

gapminder %>%
  group_by(continent, country) #"groups" part on top of the data frame
#summarize function peels back layers...
#run it in the console to see what is really going on
...
```{r}
rank(c(4.5, 2.3, 5))
#these can't be calculated independently
...
#lag takes a vector and shifts it back by 1.. creates first value NA, if you wanna use today's river flow to predict tomorrow's you would lag the data
```
...
gapminder %>%
  group_by(country) %>%
  mutate(growth = pop - pop[year == 1972]) #brackets subset
```
...
cda <- gapminder %>%
  ...
  my_df %>% filter(country =="Canada") #sanity check?
  summary(my_df$rel_gdp) #values typically lower than 1
