...
```{r}
rbind(lotr1, lotr2) %>% str() #another level added to the 
bind_rows(lotr1, lotr2) %>% str() #no assumptions made.. converted to a character vector 
as.character(5)
```
...
gap_small %>% 
  droplevels() %>%
  str() #factors present but #levels reduced 
...
ggplot(aes(year, lifeExp)) +
  geom_line(aes(group=country, colour=country))
#orders the legend by which line is at the top at the right side ... useful for B&W journals
```
...
```{r}
cont %>% 
  fct_lump(n=2) %>% #keep the top 2 frequent levels but lump everything else into 1
  qplot()
```
