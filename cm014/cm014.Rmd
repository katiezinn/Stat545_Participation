...
```
 #fitting a curve through the data so we can make predictions
...
__Formula__:
~ = an equal side, or predict the left hand side using the variables on the right hand side
...
augment(my_lm, newdata=tibble(gdpPercap=c(400, 500, 600))) #binds new columnds onto the original data frame 
 #we can specify new data here

glance(my_lm) #tells us us how well the model fits to the data
```
